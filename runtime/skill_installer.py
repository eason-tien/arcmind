...
class SkillInstaller:
    ...
    def install(self, url: str) -> dict:
        ...
        try:
            ...
            # 9. Auto-register to tools_registry.json
            try:
                registry_path = self.skills_dir.parent / "config" / "tools_registry.json"
                if registry_path.exists():
                    ...
                    if name not in reg["skills"]:
                        ...
                        for inp in manifest.get("inputs", []):
                            if inp.get("name") == "action" and inp.get("options"):
                                actions = inp["options"]
                        reg["skills"][name] = {
                            "description": manifest.get("description", f"Installed from {url}"),
                            "setup": f"Installed via: {url}",
                            "usage": {"skill_name": name, "inputs": {"action": actions[0] if actions else "run"}},
                            "actions": actions or ["run"],
                        }
                        registry_path.write_text(
                            json.dumps(reg, indent=2, ensure_ascii=False)
                        )
                        logger.info("[SkillInstaller] Registered to tools_registry: %s", name)
            except Exception as e:
                logger.warning("[SkillInstaller] tools_registry update failed: %s", e)
