...
class CapabilitySelector:
    ...
    def _build_index(self):
        ...
        try:
            ...
            for skill_entry in skill_manager.list_skills():
                skill_name = skill_entry["name"] if isinstance(skill_entry, dict) else str(skill_entry)
                info = skill_manager.get_manifest(skill_name)
    ...
    def select_all(self, intent: str, top_k: int = 10,
                   ...
                   agent_id: str | None = None) -> list[SelectionResult]:
        """
        不分類型，語義匹配所有能力。
        agent_id: V3.1 — 過濾掉該 agent 無權使用的 skills。
        """
        self._ensure_initialized()
        return self._query(intent, kind_filter=None, top_k=top_k,
                           min_score=min_score, agent_id=agent_id)
    ...
    def _query(self, intent: str, kind_filter: str | None = None,
        ...
        # V3.1: Resolve agent's allowed skills for filtering
        _allowed_skills: set | None = None
        if agent_id:
            try:
                from runtime.agent_registry import agent_registry
                persona = agent_registry.get(agent_id)
                if persona and persona.allowed_skills and "__all__" not in persona.allowed_skills:
                    _allowed_skills = set(persona.allowed_skills)
            except Exception:
                pass
            ...
            if intent_vec:
                for entry in self._index:
                    if kind_filter and entry.kind != kind_filter:
                        continue
                    # V3.1: Skip skills this agent can't access
                    if _allowed_skills and entry.kind == "skill":
                        skill_name = entry.metadata.get("skill_name", entry.name)
        ...
        kw_results = self._keyword_fallback(intent, kind_filter, top_k * 2,
                                            _allowed_skills=_allowed_skills)
    ...
    def _keyword_fallback(self, intent: str, kind_filter: str | None,
                          top_k: int,
                          _allowed_skills: set | None = None) -> list[SelectionResult]:
        """
