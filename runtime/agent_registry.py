...
class AgentPersona:
    ...
    allowed_skills: List[str] = field(default_factory=list)  # V3.1: Per-agent skill ACL
    enabled: bool = True
...
class AgentRegistry:
        ...
        try:
            ...
            for agent_def in data.get("agents", []):
                persona = AgentPersona(
                    ...
                    allowed_skills=agent_def.get("allowed_skills", []),
    ...
    def format_roster(self) -> str:
        ...
        for a in self._agents.values():
            ...
            if a.allowed_skills:
                lines.append(f"  Skills: {', '.join(a.allowed_skills)}")
            lines.append("")
    ...
    def add_agent(
        ...
        persona = AgentPersona(
            ...
            allowed_tools=["web_search", "memory_query", "read_file"],  # 安全預設，不含 run_command
            default_model=model,
