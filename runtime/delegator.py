...
_CAPABILITY_KEYWORDS: dict[str, list[str]] = {
    ...
    # SRE (includes deployment/monitoring — devops merged into sre in v0.9.3)
    "deployment": [
    ...
    # security (安全掃描/滲透/防火牆 → security agent)
    "security": [
        "安全", "security", "漏洞", "vulnerability", "渗透", "滲透",
        "penetration", "owasp", "xss", "sql injection", "cve",
        "扫描", "描掃", "scan", "端口", "port", "nmap", "nikto", "lynis",
        "白帽", "whitehat", "检测", "檢測", "防火墙", "防火牆",
        "firewall", "ssl", "tls", "dns", "入侵", "intrusion",
    ],
    # auditor (品質審計/幻覺檢測/完成品驗收 → auditor agent)
    "confirm_execution": [
        "审计", "審計", "audit", "核查", "驗收", "验收",
        "品質檢查", "品质检查", "quality check", "幻覺", "幻觉", "hallucination",
        "確認完成", "确认完成", "confirm", "檢驗", "检验",
    ],
...
class Delegator:
    ...
    def route_multi(self, command: str) -> Optional[DelegationPlan]:
        ...
        if not is_multi or len(scores) < 2:
            # Single agent — use already computed best score to avoid double computation
            best_cap, best_score = scores[0]
            match = self._find_best_agent(best_cap)
            if not match:
                return None
