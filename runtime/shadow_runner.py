...
class ShadowRunner:
    ...
    def test(self, test_commands: list[str] | None = None) -> dict:
        ...
        # Use json.dumps to safely escape the path in the generated Python code
        import json as _json
        safe_dir = _json.dumps(str(self.shadow_dir))
        result = self._run_test(
            "syntax_check",
            [
                sys.executable, "-c",
                "import py_compile, glob, pathlib, sys, json; "
                f"shadow_dir = {safe_dir}; "
                "files = glob.glob(shadow_dir + '/**/*.py', recursive=True); "
                "venv_prefix = str(pathlib.Path(shadow_dir) / '.venv'); "
                "errors = [f for f in files "
                "if not f.startswith(venv_prefix) "
                "and py_compile.compile(f, doraise=False) is None]; "
