...
class BaseProvider:
    ...
    def complete(self, model: str, messages: list[dict],
                 system: str | None, max_tokens: int, **kwargs) -> ModelResponse:
...
class OpenAICompatibleProvider(BaseProvider):
    ...
    def complete(self, model: str, messages: list[dict],
        ...
        # OpenAI gpt-4o/gpt-5+ models require max_completion_tokens instead of max_tokens
        _use_new_param = (
            self.name == "openai"
            and any(model.startswith(p) for p in ("gpt-4o", "gpt-5", "o1", "o3", "o4"))
        )
        token_param = (
            {"max_completion_tokens": max_tokens} if _use_new_param
...
class ModelRouter:
    ...
    def complete(self, prompt: str, system: str | None = None,
        ...
        for m_str in chain:
            ...
            if not provider:
                continue
            # Recompute max_tokens per fallback model (different models have different limits)
            fallback_max = max_tokens or self._DEFAULT_MAX.get(m_str, 4096)
            try:
                logger.info("Calling fallback %s model=%s max_tokens=%d", provider_name, model_id, fallback_max)
                return provider.complete(model_id, msg_list, system, fallback_max)
