...
class EventType(str, Enum):
    ...
    # V2 Phase 1: Project events
    PROJECT_CREATED = "project_created"
    PROJECT_STATUS_CHANGED = "project_status_changed"
    PROJECT_COMPLETED = "project_completed"
    # V2 Phase 2: PM result delivery
    PM_RESULT_READY = "pm_result_ready"
    # V3: Governance events
...
class EventBus:
    ...
    def __init__(self, max_queue_size: int = 5000):
        ...
        self._started = False
        self._pending_events: list = []  # events before start()
        ...
        # Event loop reference for thread-safe cross-thread emit
    ...
    def subscribe(self, event_type: EventType, handler: EventHandler) -> None:
        ...
        logger.info("[EventBus] subscribed: %s → %s",
                    event_type, getattr(handler, '__name__', str(handler)))
    ...
    def emit(self, event: Event) -> None:
        ...
        if self._running and self._queue is not None:
            ...
            # Thread-safe: detect if we're in the event loop thread
            try:
                asyncio.get_running_loop()
                # In event loop thread — direct put is safe
                try:
                ...
                # In a worker thread — use call_soon_threadsafe
                if self._loop and self._loop.is_running():
                    try:
                        self._loop.call_soon_threadsafe(
                            self._safe_enqueue, item, event
                        )
                    except RuntimeError:
                        logger.warning("[EventBus] Loop closed, dropping event %s", event.id)
                        self._dead_letters.append(event)
                        self._trim_dead_letters()
                else:
                    logger.warning("[EventBus] No loop ref, event %s dropped (type=%s)", event.id, event.type.value)
                    self._dead_letters.append(event)
                    self._trim_dead_letters()
        else:
            ...
            try:
                ...
                logger.warning("[EventBus] No event loop, event %s dropped (type=%s)", event.id, event.type.value)
    ...
    async def start(self) -> None:
        ...
        # Flush events emitted before start
        for pending_event in self._pending_events:
            if isinstance(pending_event, Event):
                await self._dispatch(pending_event)
            elif isinstance(pending_event, (list, tuple)) and len(pending_event) >= 2:
                # Legacy tuple format (type, payload) — wrap into Event
                try:
                    evt = Event(type=pending_event[0], source="pending_flush", payload=pending_event[1] if isinstance(pending_event[1], dict) else {})
                    await self._dispatch(evt)
                except Exception as e:
                    logger.warning("[EventBus] Failed to flush pending event: %s", e)
            else:
                logger.warning("[EventBus] Unknown pending event format: %s", type(pending_event))
    ...
    def _safe_enqueue(self, item: tuple, event: Event) -> None:
        """Thread-safe enqueue called in event loop thread via call_soon_threadsafe."""
        try:
            self._queue.put_nowait(item)
        except asyncio.QueueFull:
            logger.warning("[EventBus] Queue full in _safe_enqueue, dropping event %s", event.id)
            self._dead_letters.append(event)
            self._trim_dead_letters()
