...
class PersonaInjector:
    ...
    def build_system_prompt(
        ...
        if not compact:
            # Layer 2: AGENTS (behavioral rules) — full in non-compact mode
            ...
            # Layer 2b: AGENTS compact — inject core behavioral rules only
            agents = self.loader.get_agents()
            if agents:
                # Extract first section (before first '---' or first 500 chars)
                sep_idx = agents.find("\n---")
                compact_agents = agents[:sep_idx] if sep_idx > 0 else agents[:500]
                if compact_agents.strip():
                    sections.append(f"## 行為規範（精簡）\n{compact_agents.strip()}")

            # Layer 3b: TOOLS compact — inject skill name + description (lazy loading)
            # JARVIS calls get_skill_info(skill_name) for full usage when needed
            try:
                ...
                if skills:
                    ...
                    for s in skills:
                        name = s["name"]
                        desc = s.get("manifest", {}).get("description", "")
                        skill_lines.append(f"- {name}: {desc}")
                    sections.append(
                        f"## 可用 Skills（{len(skills)} 個）\n"
                        f"使用 `invoke_skill(skill_name, inputs)` 調用。"
                        f"使用 `get_skill_info(skill_name)` 查看完整用法。\n\n"
                        + "\n".join(skill_lines)
                    )
            except Exception as e:
                logger.debug("[PersonaInjector] compact skills skipped: %s", e)
