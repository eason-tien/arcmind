...
class DeliveryQueue:
    ...
    def __init__(self):
        ...
        self._lock = threading.Lock()  # 保護 _callbacks/_queues 的並發存取
...
async def _handle_system_command(command: str, ctx: SessionContext) -> str:
    ...
    if command == "/cancel":
        ...
        # Text-based fallback for non-Telegram channels
        try:
            import json as _json
            from pathlib import Path as _Path
            reg_path = _Path(__file__).parent.parent / "config" / "tools_registry.json"
            ...
            lines.append(f"\n共 {len(skills)} 個工具。在 Telegram 使用 /tools 可互動瀏覽。")
            return "\n".join(lines)
        except Exception as e:
            return f"⚠️ 無法載入工具列表: {e}"
...
async def _handle_agent_task(
    ...
    try:
        ...
        logger.exception("[Gateway] error processing agent task: %s", e)
        return f"⚠️ 內部錯誤: {e}"
...
async def chat_audio_endpoint(
    ...
    _ALLOWED_AUDIO_SUFFIXES = {".webm", ".wav", ".mp3", ".ogg", ".m4a", ".flac"}
    raw_suffix = Path(audio.filename).suffix.lower() if audio.filename else ".webm"
    suffix = raw_suffix if raw_suffix in _ALLOWED_AUDIO_SUFFIXES else ".webm"
    ...
    try:
        ...
        # Cleanup: 清理上傳的暫存檔和 TTS 產生的音訊檔
        if temp_file.exists():
            temp_file.unlink()
        try:
            if 'tts_path' in dir() and tts_path and Path(tts_path).exists():
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    if _settings.api_key:
        qs_key = websocket.query_params.get("api_key", "")
        if qs_key != _settings.api_key:
    ...
    try:
        ...
        async def ws_delivery_callback(out_msg: OutboundMessage):
            try:
                ...
                logger.warning("[Gateway/WS] delivery callback failed for session=%s: %s",
                               out_msg.session_id, _ws_err)
        ...
        while True:
            ...
            # Filter client data to prevent session_id/user_id hijacking
            safe_data = {k: v for k, v in data.items()
                         if k not in ("session_id", "user_id", "text")}
            ...
            def _log_ws_task_error(t):
                if not t.cancelled() and t.exception():
                    logger.warning("[Gateway/WS] background task error: %s", t.exception())
            task.add_done_callback(_log_ws_task_error)
