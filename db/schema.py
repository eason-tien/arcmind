...
class Heartbeat_(Base):
    """系統組件心跳監控"""
    __tablename__ = "am_heartbeats"

    id                    = Column(Integer, primary_key=True, autoincrement=True)
    source_id             = Column(String(128), nullable=False, index=True)    # 組件唯一標識
    source_type           = Column(String(32), default="agent")               # agent/service/system/worker
    status                = Column(String(32), default="alive")               # alive/dead/degraded/unknown
    is_alive              = Column(Boolean, default=True)
    timestamp             = Column(DateTime, default=datetime.utcnow, index=True)
    metadata_json         = Column(Text, default="{}")                         # 額外信息JSON
    response_time_ms      = Column(Float, nullable=True)                        # 響應時間(毫秒)
    error_message         = Column(Text, nullable=True)                         # 錯誤信息
    consecutive_failures  = Column(Integer, default=0)                         # 連續失敗次數
    total_heartbeats      = Column(Integer, default=1)                          # 總心跳次數


# ── Init ─────────────────────────────────────────────────────────────────────
import db.harness_schema as _harness_schema  # noqa: F401 — register harness tables
import db.project_schema as _project_schema  # noqa: F401 — register project tables (V2)
import db.v3_schema as _v3_schema            # noqa: F401 — register V3 governance/closure tables
